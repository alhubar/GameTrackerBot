...
**When we play** is a 24-hour histogram on `/server` showing the shape of the last 90 days: one block per hour of the day, scaled against the server's own busiest hour, with that hour named above it. It is the one thing here read off the clock a session happened on rather than how long it ran, and it exists to answer the question `/event` has to guess at — pick a time people are already around and they turn up. A session counts toward *every* hour it covered, not just the one it started in, so an evening that runs from 20:00 to midnight says people were there at 23:00. An hour with nothing in it is drawn differently from an hour with a little, so a genuinely empty slot is visible as one.

The hours are wall-clock hours in `SERVER_TIMEZONE`, which is a single zone and unrelated to the event timezone presets — it defaults to UTC, so set it to wherever most of your members live or the peak will read hours out. The zone is named under the histogram either way. This block counts hours with nobody's name attached, so like the server's total gaming time it includes members who have since left, and it is omitted entirely until there is play in the window.
